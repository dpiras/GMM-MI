...
def single_fit(X, n_components, reg_covar=1e-15, tol=1e-6, max_iter=10000, 
    ...
    random_state : int, default=None
        Random seed used to initialise the GMM model. Not used if initial GMM parameters are provided.
...
def GMM_MI(X, n_folds=3, n_inits=5, init_type='random_sklearn', reg_covar=1e-15, 
    ...
    for n_components in range(1, max_components+1):
        ...
        if not converged:
            metric = select_best_metric(X=X, current_results_dict=current_results_dict, select_c=select_c,
