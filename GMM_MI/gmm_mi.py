...
def cross_validation(X, n_components=1, n_folds=5, n_inits=5, max_iter=10000, 
    ...
    for r in range(n_inits):
        ...
        for k_idx, (train_indices, valid_indices) in enumerate(kf.split(X)):
            ...
            fitted_gmm = GMM(n_components=n_components, reg_covar=reg_covar, 
                            ...
                            means_init=m_init, precisions_init=p_init).fit(X_training, val_set=X_validation)
            ...
            all_loss_curves.append(np.copy(fitted_gmm.train_loss))
            all_loss_curves.append(np.copy(fitted_gmm.val_loss))
    ...
    results_dict = {'best_seed': best_seed, 'best_fold_in_init': best_fold_in_init, 
                   ...
                   'all_ws': all_ws, 'all_ms': all_ms, 'all_cs': all_cs, 'all_ps': all_ps}
    return results_dict        
    
def perform_bootstrap(X, n_bootstrap, best_components, 
                      reg_covar, tol, max_iter, random_state,
                      w_init, m_init, p_init, MC_samples
                     ):
    """
    TODO
    """
    ...
    for i in range(n_bootstrap):
        ...
        gmm = GMM(n_components=best_components, reg_covar=reg_covar, 
                    tol=tol, max_iter=max_iter, 
...
def GMM_MI(X, n_folds=5, n_inits=5, 
    ...
    for n_components in range(1, components_range+1):
        ...
        if converged:
            ...
            all_cs = results_dict[best_components]['all_cs']
            ...
            c_init = all_cs[best_seed, best_fold_in_init]
            p_init = all_ps[best_seed, best_fold_in_init]
            
            if bootstrap:
                MI_mean, MI_std = perform_bootstrap(X, n_bootstrap=n_bootstrap, best_components=best_components, 
                ...
                gmm = GMM(n_components=best_components, reg_covar=reg_covar, 
                            ...
                            means_init=m_init, precisions_init=p_init).fit(X)
                MI_mean = gmm.estimate_MI_MC(MC_samples=MC_samples)
                MI_std = None
            break
